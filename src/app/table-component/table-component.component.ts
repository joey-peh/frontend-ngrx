...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatTableModule, HttpClientModule, MatCardModule, MatButtonModule,
    MatButtonModule, HttpClientModule, MatDialogModule, MatIconModule, MatSlideToggleModule, FormsModule, FlexLayoutModule],
  templateUrl: './table-component.component.html',
  styleUrls: ['./table-component.component.css', '../app.component.css']
})
export class TableComponentComponent implements OnInit, AfterViewInit {
  ...
  constructor(private readonly store: Store, public dialog: MatDialog, private localStore: LocalService) {
    //If local storage has dark mode key, then set. Else it will be false by default.
    let tmp = this.localStore.getData(this.isDarkModeKey);
    if (tmp != undefined) {
      this.isDarkMode = JSON.parse(tmp);
    }
  }
  ...
  readonly persons$ = this.store.select(userFeature.selectAllUsers);
  private isDarkModeKey: string = "isDarkMode";

  isDarkMode: boolean = false;

  ngOnInit(): void {
    this.store.dispatch(userAction.getUserList());
    this.persons$.subscribe((payload: any) => {
